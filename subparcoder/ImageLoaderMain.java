...
public class ImageLoaderMain extends JavaPlugin {
    ...
    public final boolean onCommand(@NotNull final CommandSender sender, @NotNull final Command cmd, @NotNull final String label, @NotNull final String[] args) {
        ...
        if (!(sender instanceof Player player)) {
            sender.sendMessage(ImageLoaderMain.imageLoaderMessage("Only players can use this command!", true));
            return true;
        }
        switch(label.toLowerCase()) {
            ...
            case "imgload" -> {
                if (args.length != 3) {
                    player.sendMessage(ImageLoaderMain.imageLoaderMessage("Usage: /imgload <image folder> <image format> <replacer> (i.e /imgload images image_<i>.jpg <i>)", true));
                } else {
            ...
            case "getmap" -> {
                if (args.length != 2) {
                    player.sendMessage(ImageLoaderMain.imageLoaderMessage("Usage: /getmap <fps (max fps = 20)> <threads> (i.e /getmap 20 2))", true));
                } else {
            ...
            case "getfillmap" -> {
                if (args.length != 2) {
                    player.sendMessage(ImageLoaderMain.imageLoaderMessage("Usage: /getfillmap <fps (max fps = 20)> <threads> (i.e /getfillmap 20 2))", true));
                } else {
            ...
            case "setsearches" -> {
                if (args.length != 1) {
                    player.sendMessage(ImageLoaderMain.imageLoaderMessage("Usage: /setsearches <max searches> (i.e /setsearches 8))", true));
                } else {
    ...
    private void asyncImageLoad(@NotNull final String[] args, @NotNull final Player player) {
        ...
        player.sendMessage(ImageLoaderMain.imageLoaderMessage("Attempting to load the images to memory!", false));
        new BukkitRunnable() {
            @Override
            public void run() {
                final byte[][] bytes = ImageManagerUtil.getBytes(args[0], args[1], args[2]);
                if (bytes == null || !ImageManagerUtil.setLoadedBytes(bytes)) {
                    player.sendMessage(ImageLoaderMain.imageLoaderMessage(ImageManagerUtil.getLastError(), true));
                } else {
                    player.sendMessage(ImageLoaderMain.imageLoaderMessage("Successfully loaded the images into memory!", false));
                }
            }
    ...
    private void giveMap(@NotNull final String[] args, @NotNull final Player player) {
        ...
        if (!ImageManagerUtil.hasLoadedBytes()) {
            player.sendMessage(ImageLoaderMain.imageLoaderMessage("Please make sure there are loaded images first! (/imgload)", true));
            return;
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                if (atomicArray.isArrayUpdated()) {
                    ...
                    if (meta == null) {
                        player.sendMessage(ImageLoaderMain.imageLoaderMessage("ItemMeta was null!", true));
                        cancel();
                        return;
                    }
                    ...
                    player.sendMessage(ImageLoaderMain.imageLoaderMessage("Made map!", false));
                    cancel();
                }
            }
    ...
    private void setMaxAdjacentSearches(@NotNull final String[] args, @NotNull final Player player) {
        ...
        try {
            ...
            player.sendMessage(ImageLoaderMain.imageLoaderMessage("Could not parse the max searches! (It has to be an integer)", true));
            return;
        }
        if (maxSearches < 1) {
            player.sendMessage(ImageLoaderMain.imageLoaderMessage("Max searches has to be bigger than 0! (i.e /setsearches 8)", true));
            return;
        }
        if (maxSearches > 10) {
            player.sendMessage(ImageLoaderMain.imageLoaderMessage("Warning: Having more than a 10x10 area of maps can significantly lag your server!", true));
        }
        MAX_ADJACENT_SEARCHES = maxSearches;
        player.sendMessage(ImageLoaderMain.imageLoaderMessage("Set MAX_ADJACENT_SEARCHES to " + maxSearches, false));
    }
    private void getMaxAdjacentSearches(@NotNull final Player player) {
        ...
        Objects.requireNonNull(player, "Player can not be null!");
        player.sendMessage(ImageLoaderMain.imageLoaderMessage("Max adjacent searches is " + MAX_ADJACENT_SEARCHES, false));
    }
    public static String imageLoaderMessage(@NotNull final String message, final boolean error) {
